...
> `harness` · 34 files (python×34) · 35 classes · 82 functions · ~7,474 source lines
...
> Volatility Breakout adapter — wraps volatility_breakout.signals.generator. Uses `filtered_strength` as confidence (aligned with MR/TF adapters).
...
- `class HarnessTradingDB`  — Interface to the unified harness paper trading database.
  methods: `upsert_position`, `get_position`, `get_all_positions`, `close_position`, `sync_from_alpaca`, `record_trade`, `get_trades`, `total_realized_pnl`, +1 more
